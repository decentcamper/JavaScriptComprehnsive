...
EGeoXml.prototype.createPolyline = function(points,color,width,opacity,pbounds,name,desc) {
  ...
  {
    if (this.isMapplet)
      // Maps API (not a mapplet)
      thismap.openInfoWindowHtml(p.getVertex(Math.floor(p.getVertexCount()/2)),html,iwoptions);
    else
    {
      // Mapplet
      p.getVertexCountAsync(function (count)
      {
        p.getVertexAsync(Math.floor(count / 2), function (vertex)
        {
          thismap.openInfoWindowHtml(vertex, html, iwoptions);
        });
      });
    }
  });
...
EGeoXml.prototype.parse = function() {
  ...
  for (u=0; u<this.urls.length; u++) {
    if (typeof _IG_FetchContent == 'undefined')
      ...
      _IG_FetchContent(this.urls[u], function(doc) {that.processing(doc)});
  }
};
